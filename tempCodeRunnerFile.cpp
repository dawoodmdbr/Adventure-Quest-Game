...
public:
    ...
    void placeHurdle(int row, int col, char symbol)
    {
        if (isValidCell(row, col) && grid[row][col] == EMPTY)
            ...
            cout << "Invalid cell for hurdle." << endl;
    }
...
void playGame(int boardSize)
{
    ...
    while (true)
    {
        ...
        cout << p1.name << "'s turn:" << endl << "1. Move" << endl << "2. Buy & Place Item" << endl << "Choose: ";
        int choice;
        cin >> choice;
        if (choice == 1)
            p1.move(board, true);
        ...
        cout << p2.name << "'s turn:" << endl << "1. Move" << endl << "2. Buy & Place Item" << endl << "Choose: ";
        cin >> choice;
        if (choice == 1)
            p2.move(board, false);
        else if (choice == 2)
